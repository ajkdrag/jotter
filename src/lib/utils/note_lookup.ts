...
export function resolve_existing_note_path(notes: NoteMeta[], target_path: string): string | null {
  ...
  const same_folder = matches.filter((n) => parent_folder_path(n.path).toLowerCase() === target_parent_key)
  ...
  const deterministic = [...(same_folder.length ? same_folder : matches)].sort((a, b) =>
