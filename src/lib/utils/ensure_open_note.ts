...
function next_untitled_name_in_folder(
  notes: NoteMeta[],
  ...
  const pattern = new RegExp(
    `^${folder_path.replace(/[/\\^$*+?.()|[\]{}]/g, "\\$&")}Untitled-(\\d+)\\.md$`,
  );
...
function create_untitled_open_note(args: {
  notes: NoteMeta[];
  now_ms: number;
}): OpenNoteState {
...
export function create_untitled_open_note_in_folder(args: {
  ...
  const path_with_folder = args.folder_prefix
    ? `${args.folder_prefix}/${name}`
    : name;
