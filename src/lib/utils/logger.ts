...
function log_at(level: keyof Logger, msg: string) {
  void get_logger().then((l) => { l[level](msg) })
}

export const logger = {
  error(msg: string) { log_at('error', msg) },
  warn(msg: string) { log_at('warn', msg) },
  info(msg: string) { log_at('info', msg) },
