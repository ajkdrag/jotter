...
export function create_editor_sync_reactor(
  ...
  let last_link_syntax: 'wikilink' | 'markdown' = ui_store.editor_settings.link_syntax
  ...
  return $effect.root(() => {
    $effect(() => {
      ...
      const should_open = open_note.meta.id !== last_note_id || link_syntax !== last_link_syntax
