...
export function register_vault_actions(input: ActionRegistrationInput) {
  ...
  registry.register({
    ...
    execute: async () => {
      ...
      const result = await services.vault.change_vault_by_path(path_result.path)
  ...
  registry.register({
    ...
    execute: async (vault_id: unknown) => {
      ...
      const selected_vault_id = vault_id as Parameters<typeof services.vault.change_vault_by_id>[0]
