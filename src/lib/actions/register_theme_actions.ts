...
export function register_theme_actions(input: ActionRegistrationInput) {
  ...
  registry.register({
    ...
    execute: async (args: unknown) => {
      ...
      stores.ui.set_active_theme_id(new_theme.id);
      await services.theme.save_user_themes(updated);
      await services.theme.save_active_theme_id(new_theme.id);
  ...
  registry.register({
    ...
    execute: async (theme_id: unknown) => {
      ...
      const source = all.find((t) => t.id === theme_id);
      if (!source) return;
  ...
  registry.register({
    ...
    execute: async (theme_id: unknown) => {
      ...
      if (stores.ui.active_theme_id === theme_id) {
        ...
        await services.theme.save_active_theme_id(DEFAULT_THEME_ID);
      }
      await services.theme.save_user_themes(updated);
      persisted_theme_snapshot = null;
    },
