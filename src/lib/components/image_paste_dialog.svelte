...
<Dialog.Root {open} onOpenChange={(value: boolean) => { if (!value && !is_busy) on_cancel() }}>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description>
        {error ? `Failed to save image: ${error}` : 'Enter a filename for the image.'}
      </Dialog.Description>
    </Dialog.Header>
    ...
    {#if !error}
      <div class="space-y-4">
        ...
        <Input
          ...
          type="text"
          value={filename}
          onchange={(e: Event & { currentTarget: HTMLInputElement }) => { on_update_filename(e.currentTarget.value); }}
          oninput={(e: Event & { currentTarget: HTMLInputElement }) => { on_update_filename(e.currentTarget.value); }}
