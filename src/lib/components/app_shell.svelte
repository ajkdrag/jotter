<script lang="ts">
  ...
  const vault_bootstrap = use_flow_handle(stable_app.flows.vault_bootstrap)
  ...
  const vault_selection_loading = $derived(
    vault_bootstrap.snapshot.matches('starting') || change_vault.snapshot.matches('changing')
  )
...
{#if !has_vault}
  <div class="mx-auto max-w-[65ch] p-8">
    <VaultSelectionPanel
      ...
      error={vault_bootstrap.snapshot.context.error ?? change_vault.snapshot.context.error}
