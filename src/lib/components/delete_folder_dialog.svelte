<script lang="ts">
  ...
  type DeleteFolderDialogState = 'idle' | 'fetching_stats' | 'confirming' | 'deleting' | 'error'
  ...
  const normalized_state = $derived((status as DeleteFolderDialogState) ?? 'idle')
...
<Dialog.Root open={open} onOpenChange={handle_open_change}>
  <Dialog.Content class="max-w-md">
    ...
    <Dialog.Footer>
      ...
      {:else}
        <Button variant="destructive" onclick={on_confirm} disabled={is_loading}>
