...
export class FolderService {
  ...
  async create_folder(parent_path: string, folder_name: string): Promise<FolderMutationResult> {
  ...
  async load_delete_stats(folder_path: string): Promise<FolderDeleteStatsResult> {
    const vault_id = this.vault_store.vault?.id
    if (!vault_id) {
      return { status: 'skipped' }
    }
  ...
  async delete_folder(folder_path: string): Promise<FolderMutationResult> {
    ...
    try {
      ...
      const contains_open_note = this.editor_store.open_note?.meta.path.startsWith(folder_prefix) ?? false
  ...
  async rename_folder(folder_path: string, new_path: string): Promise<FolderMutationResult> {
    const vault_id = this.vault_store.vault?.id
  ...
  async load_folder(path: string, expected_generation: number): Promise<FolderLoadResult> {
    const vault_id = this.vault_store.vault?.id
    if (!vault_id) {
      return { status: 'skipped' }
    }
