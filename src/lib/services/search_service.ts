...
export class SearchService {
  ...
  async suggest_wiki_links(query: string): Promise<WikiSuggestionsResult> {
    ...
    try {
      const results = await this.search_port.suggest_wiki_links(vault_id, trimmed, 15)
      return { status: 'success', results }
    } catch (error) {
      const message = error_message(error)
  ...
  search_commands(query: string): OmnibarItem[] {
    ...
    if (!q) {
      return COMMANDS_REGISTRY.map((command) => ({ kind: 'command' as const, command, score: 0 }))
    }

    return COMMANDS_REGISTRY
      .map((command) => ({ kind: 'command' as const, command, score: score_command(q, command) }))
  ...
  search_settings(query: string): OmnibarItem[] {
    ...
    return SETTINGS_REGISTRY
      .map((setting) => ({ kind: 'setting' as const, setting, score: score_setting(q, setting) }))
      .filter((item) => item.score > 0)
      .sort((a, b) => b.score - a.score)
  }
  ...
  async search_omnibar(raw_query: string): Promise<OmnibarSearchResult> {
    ...
    if (parsed.domain === 'commands') {
      const items = [...this.search_commands(parsed.text), ...this.search_settings(parsed.text)]
