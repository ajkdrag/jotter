...
async function ensure_settings_dir(
...
async function read_settings_file(
  settings_dir: FileSystemDirectoryHandle,
): Promise<Record<string, unknown>> {
  let text: string;
  try {
    const file_handle = await settings_dir.getFileHandle(SETTINGS_FILE, {
      create: false,
    });
...
async function write_settings_file(
  settings_dir: FileSystemDirectoryHandle,
