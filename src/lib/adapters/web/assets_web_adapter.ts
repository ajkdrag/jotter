...
function image_extension(input: {
  mime_type: string;
  file_name: string | null;
}): string {
  const from_name = input.file_name
    ?.split("/")
    .pop()
    ?.split(".")
    .pop()
...
function create_asset_path(
  ...
  const directory =
    note_parts.length > 0
      ? `${note_parts.join("/")}/${attachment_folder}`
      : attachment_folder;
...
async function resolve_from_opfs(
  vault_id: VaultId,
  ...
  try {
    ...
    blob_url_cache.set(`${vault_id}:${asset_path}`, blob_url);
    return blob_url;
  } catch (e) {
    throw new Error(
      `Failed to resolve asset URL: ${asset_path}. ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
...
export function create_assets_web_adapter(): AssetsPort {
  return {
    resolve_asset_url(
      ...
      const cached_url = blob_url_cache.get(`${vault_id}:${asset_path}`);
      if (cached_url) return cached_url;

      return resolve_from_opfs(vault_id, asset_path);
    },
    async write_image_asset(vault_id, input) {
      try {
        ...
        const asset_path = create_asset_path(
          String(input.note_path),
          input.image,
          options,
        );
        const { dir, file_name } = await ensure_path_from_root(
          root,
          asset_path,
        );
        const file_handle = await dir.getFileHandle(file_name, {
          create: true,
        });
        ...
        throw new Error(
          `Failed to write image asset. ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    },
  };
