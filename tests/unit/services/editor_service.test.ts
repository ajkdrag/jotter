...
describe('EditorService', () => {
  it('mounts and opens buffers with session configs and hooks', async () => {
    ...
    let invocation = 0
    const { service, editor_store, start_session, session_configs } = create_setup(() => {
      invocation += 1
  ...
  it('applies markdown updates only for active session generation', async () => {
    ...
    expect(editor_store.open_note?.markdown).toBe(as_markdown_text('# updated'))
  })
  ...
  it('flushes session markdown and syncs editor store', async () => {
    ...
    const { service, editor_store } = create_setup(() => Promise.resolve(session))
    const root = {} as HTMLDivElement
    const note = create_open_note('docs/alpha.md', '# Alpha')
    ...
    expect(editor_store.open_note?.markdown).toBe(as_markdown_text('# Flushed'))
  })
