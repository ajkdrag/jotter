...
describe('FolderService', () => {
  it('loads first page and reports pagination metadata', async () => {
    ...
    expect(notes_store.notes.map((note) => note.path)).toEqual(['note-001.md', 'note-002.md'])
  ...
  it('loads additional pages additively', async () => {
    ...
    const many_notes = Array.from({ length: 205 }, (_, index) => create_note(index + 1))
    ...
    const second = await service.load_folder_page('', 200, vault_store.generation)
