...
describe("GitService", () => {
  ...
  it("init_repo calls port and enables store", async () => {
    const { service, init_repo, git_store, op_store } = create_harness();
    await service.init_repo();
  ...
  it("auto_commit multiple files uses count in message", async () => {
    const { service, stage_and_commit } = create_harness();
  ...
  it("auto_commit handles errors", async () => {
    const { service, stage_and_commit, git_store, op_store } = create_harness();
  ...
  it("create_checkpoint commits with description", async () => {
    const { service, stage_and_commit } = create_harness();
    await service.create_checkpoint("My checkpoint");
    expect(stage_and_commit).toHaveBeenCalledWith(
      expect.anything(),
      "My checkpoint",
      null,
  ...
  it("restore_version calls port and refreshes status", async () => {
    const { service, restore_file, status, op_store } = create_harness();
