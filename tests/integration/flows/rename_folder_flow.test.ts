...
describe('rename_folder_flow', () => {
  it('renames folder and updates stores', async () => {
    ...
    stores.dispatch({ type: 'folders_set', folder_paths: ['old'] })
    const note_a = create_test_note('old/note-a', 'Note A')
    stores.dispatch({ type: 'notes_set', notes: [note_a] })
    ...
    expect(index_port._calls.build_index).toEqual([vault.id])
    expect(index_port._calls.remove_note).toHaveLength(0)
    expect(index_port._calls.upsert_note).toHaveLength(0)
  })
